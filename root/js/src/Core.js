...
//        console.log("width = " + $(dom).css("width"));
    ...
    {
        if(loadingStyleIndex != null) SimplePreloading.show(loadingStyleIndex);

        if(imageSetting != null)
        {
            Utility.preloadImages(imageSetting.list, loadFrame ,function(progress)
            {
                //if(loadingStyleIndex != null) SimplePreloading.updateProgress(progress, imageSetting.startWeight, imageSetting.weight, loadingStyleIndex);
            });
        }
        else loadFrame(0);
        ...
        function loadFrame(index)
        {
            if(frames.length == 0)
            {
                if(loadingStyleIndex != null && !keepLoadingLive) SimplePreloading.hide(loadingStyleIndex);
            ...
            $(frameDom).load(frames[index].url, function()
            {
                ...
                }, function(loaded, count)
                {
                    ...
                    //if(loadingStyleIndex != null) SimplePreloading.updateProgress(progress, frames[index].startWeight, frames[index].weight, loadingStyleIndex);
                }, true);
            });
        }
    };
